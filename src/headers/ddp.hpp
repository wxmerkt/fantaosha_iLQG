...
template <typename Robot>
double DDP<Robot>::getGnorm(std::vector<VecN> const &kus,
...
template <typename Robot>
DDP<Robot>::DDP(System const &sys_, double const &dt_)
    : sys(sys_), dt(dt_), num(0) {}

template <typename Robot>
bool DDP<Robot>::init(State const &x0_, std::vector<U> const &us_,
...
template <typename Robot>
void DDP<Robot>::iterate(int const &itr_max, std::vector<U> &us0) {
  std::vector<MatNM> Ks;
  std::vector<VecN> kus;
...
template <typename Robot>
void DDP<Robot>::forwards(std::vector<MatNM> const &Ks,
...
template <typename Robot>
int DDP<Robot>::backwards(std::vector<MatNM> &Ks, std::vector<VecN> &kus,
                          double const &lambda) {
  // Initialization
  VecM Qx;
  VecN Qu;
