...
class Sim {
 ...
 public:
  ...
  State get_state() const { return states[0].back(); }

  std::vector<State> const &get_states() const { return states[0]; }

  std::vector<U> const &get_inputs() const { return inputs; }
  ...
  void save(std::string path) const {
    ...
    typename std::vector<Eigen::Matrix<double, Robot::N, 1> >::const_iterator
        itr_u = inputs.begin();
