/*************************************************************************
    ...
    > Author: Taosha Fan
    > Mail: tfan1@jhu.edu
    > Created Time: Fri 22 Apr 2016 03:20:23 PM CDT
 ************************************************************************/
#ifndef _BOXQP
#define _BOXQP
#include <algorithm>
...
template <size_t N>
Eigen::Matrix<double, N, 1> clamp(Eigen::Matrix<double, N, 1> const &x,
                                  Eigen::Matrix<double, N, 1> const &xmin,
                                  Eigen::Matrix<double, N, 1> const &xmax) {
...
template <size_t N>
Eigen::MatrixXd select(Eigen::Matrix<double, N, N> const &M,
                       Eigen::Array<int, N, 1> const &istate,
                       Eigen::Array<int, N, 1> const &jstate) {
  int m = istate.sum();
  int n = jstate.sum();
  ...
  for (int i = 0, j = 0; i < N && j < m; i++)
    if (istate(i)) M1.row(j++) = M.row(i);

  for (int i = 0, j = 0; i < N && j < n; i++)
    if (jstate(i)) M2.col(j++) = M1.col(i);

  return M2;
}

template <size_t N>
Eigen::VectorXd select(Eigen::Matrix<double, N, 1> const &v,
                       Eigen::Array<int, N, 1> const &istate) {
  int n = istate.sum();
  ...
  for (int i = 0, j = 0; i < N && j < n; i++)
    if (istate(i)) v1(j++) = v(i);

  return v1;
}

template <size_t N>
void assign(Eigen::Matrix<double, N, 1> &v1, Eigen::MatrixXd const &v2,
            Eigen::Array<int, N, 1> const &istate) {
  assert(v2.rows() >= istate.sum());
  for (int i = 0, j = 0; i < N; i++)
    if (istate(i)) v1(i) = v2(j++);
}

template <size_t N>
void boxQP(Eigen::Matrix<double, N, N> const &H,
           Eigen::Matrix<double, N, 1> const &g,
           Eigen::Matrix<double, N, 1> const &xmin,
           Eigen::Matrix<double, N, 1> const &xmax,
           Eigen::Matrix<double, N, 1> const &x0,
           Eigen::Matrix<double, N, 1> &x, Eigen::MatrixXd &Hfree,
           Eigen::Array<int, N, 1> &free, int &result) {
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    for (int i = 0; i < N; i++)
      clamped(i) =
          (x(i) <= xmin(i) && grad(i) > 0) || (x(i) >= xmax(i) && grad(i) < 0);
    ...
    VecNd grad_clamped =
        g + H * ((x.array() * (clamped.template cast<double>())).matrix());
    VecNd dx = VecNd::Zero();
    assign<N>(dx,
              -llt.solve(select<N>(grad_clamped, free)) - select<N>(x, free),
              free);
