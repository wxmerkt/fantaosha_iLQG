...
class snOPT {
 public:
  ...
  struct Params {
   public:
    ...
    Params(Vec xlow_, Vec xupp_, double Flow_ = -1e20, double Fupp_ = 1e20,
           double ObjAdd_ = 0)
        : xlow(xlow_), xupp(xupp_), Flow(Flow_), Fupp(Fupp_), ObjAdd(ObjAdd_) {}
  };
 ...
 public:
  ...
  static int is_ready() { return initialized; }

  void static usrfunObj(int *Status, int *n, double x[], int *needF, int *neF,
  ...
  void static usrfunG(int *Status, int *n, double x[], int *needF, int *neF,
                      double F[], int *needG, int *neG, double G[], char *cu,
                      int *lencu, int iu[], int *leniu, double ru[],
                      int *lenru) {
    Vec xx;
    memcpy(xx.data(), x, num * sizeof(double));
    Eigen::Matrix<double, 1, num> xxT = xx.transpose();
    if (*needF > 0) F[0] = (0.5 * xxT * H * xx + xxT * h)(0);
  ...
  static int fmin(Mat const &H, Vec const &h, Params const &params, Vec &x0,
                  Vec &xmul0, Eigen::Matrix<int, N, 1> &xstate0, double &F0,
                  double &Fmul0, int &Fstate0) {
    snOPT::H = H;
    snOPT::h = h;
...
template <size_t N>
double snOPT<N>::x[snOPT<N>::num] = {0};
template <size_t N>
double snOPT<N>::xlow[snOPT<N>::num] = {0};
template <size_t N>
double snOPT<N>::xupp[snOPT<N>::num] = {0};
template <size_t N>
double snOPT<N>::xmul[snOPT<N>::num] = {0};
template <size_t N>
int snOPT<N>::xstate[snOPT<N>::num] = {0};

template <size_t N>
double snOPT<N>::F[snOPT<N>::nef] = {0};
template <size_t N>
double snOPT<N>::Flow[snOPT<N>::nef] = {0};
template <size_t N>
double snOPT<N>::Fupp[snOPT<N>::nef] = {0};
template <size_t N>
double snOPT<N>::Fmul[snOPT<N>::nef] = {0};
template <size_t N>
int snOPT<N>::Fstate[snOPT<N>::nef] = {0};

template <size_t N>
double snOPT<N>::ObjAdd = 0;
template <size_t N>
bool snOPT<N>::initialized = false;

template <size_t N>
typename snOPT<N>::Mat snOPT<N>::H = snOPT<N>::Mat::Identity();
template <size_t N>
typename snOPT<N>::Vec snOPT<N>::h = snOPT<N>::Vec::Zero();
template <size_t N>
snoptProblemA snOPT<N>::problem;
