 ...
 public:
  ...
  struct Params {
    ...
    Params(MatMM const& Q_ = MatMM::Identity(),
           ...
           MatMM const& Qf_ = 50 * MatMM::Identity())
        : Q(Q_), Qf(Qf_), R(R_) {}
  };

 public:
  bool static solve(System const& sys, double const& dt, size_t const& num,
                    Params const& params, std::vector<U> const us,
                    std::vector<Ref> const refs, std::vector<MatNM>& Ks) {
