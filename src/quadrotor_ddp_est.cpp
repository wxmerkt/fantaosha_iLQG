...
int main() {
  char file[] =
      "/media/fantaosha/Documents/JHU/Summer "
      "2015/quad_rotor_traj/traj_full_12s_1_small.mat";
...
#pragma omp parallel for reduction(+ : succeed) schedule(dynamic)
  for (int n = 0; n < NUM; n++) {
    std::vector<typename quadrotor::State> *pxrefs =
        new std::vector<typename quadrotor::State>(xrefs0);
    std::vector<typename quadrotor::U> *pus =
        new std::vector<typename quadrotor::U>(us0);
...
#pragma omp critical
    ...
    for (int i = 0; i < 4000; i += sn) {
      ...
      //			std::cout<<"time consumed is
      //"<<(T_end.tv_sec-T_start.tv_sec)+(T_end.tv_nsec-T_start.tv_nsec)/1000000000.0<<"s"<<std::endl;
      ...
      if (error.norm() < 5) {
        success = true;
        break;
      } else if (error.norm() > 150)
        break;
    }

    if (success) succeed += 1;
#pragma omp critical
    {
      std::cout
          << "============================================================"
          << std::endl;
      std::cout << quadrotor::State::diff(sim.get_state(), xrefs[0]).transpose()
                << std::endl;
      if (success)
